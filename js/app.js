...
function renderCertificate(courseId){
  ...
  const name=profile?.full_name||currentEmail();
  const issued=cert?.issued_at?new Date(cert.issued_at):new Date();
  const dateLong=issued.toLocaleDateString('en-AU',{day:'numeric',month:'long',year:'numeric'});
  return `<section class="topbar noPrint"><button class="ghost" onclick="setRoute('course','${c.id}')">← Back</button><button onclick="window.print()">Print / Save as PDF</button></section>
<div class="certWrap">
  <div class="cert">
    <div class="cert__head">
      <div class="cert__monogram">TCR</div>
      <p class="cert__org">Team Casa Rustia</p>
      <p class="cert__org-sub">AI Workplace Skills Hub</p>
    </div>
    <p class="cert__eyebrow">Certificate of Completion</p>
    <p class="cert__lead">This is to certify that</p>
    <h1 class="cert__name">${esc(name)}</h1>
    <div class="cert__rule" aria-hidden="true"><span></span><i>&#10022;</i><span></span></div>
    <p class="cert__body">has successfully completed all fourteen modules of the program</p>
    <h2 class="cert__course">${esc(c.title)}</h2>
    <p class="cert__date">Awarded on ${esc(dateLong)}</p>
    <div class="cert__foot">
      <div class="cert__sig">
        <span class="cert__sig-name">Alvin Rustia</span>
        <span class="cert__sig-line"></span>
        <span class="cert__sig-label">Program Director</span>
      </div>
      <div class="cert__seal" aria-hidden="true">
        <span class="cert__seal-inner"><span class="cert__seal-mono">TCR</span><span class="cert__seal-text">Certified</span></span>
      </div>
      <div class="cert__sig">
        <span class="cert__sig-name">Ylona Rustia</span>
        <span class="cert__sig-line"></span>
        <span class="cert__sig-label">Head of Training</span>
      </div>
    </div>
    <p class="cert__id">Certificate ID &mdash; <b>${esc(code)}</b></p>
  </div>
</div>`;
}
